...
Uses golangci-lint with config in `.golangci.yml`, in **v2 format** — the config and the binary must both be v2, and the binary must be built with the Go version `go.mod` targets (`go install github.com/golangci/golangci-lint/v2/cmd/golangci-lint@latest`), or it refuses to load the config. **CI runs it** (the `lint` job in `ci.yml`), building it the same way and from a pinned version, for that reason: a downloaded release carries whatever Go was current when it was published, which is how this repo ended up with a config nothing could run. Key enabled linters: errcheck, govet, ineffassign, staticcheck, unused, gofmt, goimports, misspell, godot, gosec, noctx, wrapcheck, exhaustive. gosimple is not among them: v2 folded its checks into staticcheck, and gofmt/goimports live in the separate `formatters` section. `govet` runs with `enable-all`, minus **fieldalignment**, which is off for the whole module rather than just for tests: two thirds of its 32 findings were table-driven tests' anonymous structs, and the rest were `Config`, `Converter`, the `-doctor` status structs and five local ones in `pdf.go` — all allocated once or a handful of times per run, so a word of padding buys nothing and field order that follows the documentation is worth more. G204 (subprocess with variable) is excluded since the mmdc and pandoc invocations are intentional. Test files have relaxed rules (no wrapcheck, gosec, errcheck). The exclusion presets v1 applied by default are named explicitly, because v2 applies none unless asked.
...
2. **mermaid.go** — `renderMermaidPNGs` rasterizes each block to a PNG (Word cannot reliably display pandoc-embedded SVG); placeholders are then rewritten to `![](…)` image references
