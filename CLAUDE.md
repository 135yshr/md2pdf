...
**input.go** reads documents: `readInput` takes the bytes from a file or, for `StdinPath` (`-`), from the `Converter.stdin` seam, and treats whitespace-only input as **empty and an error** — an empty pipe is nearly always an upstream failure, and the acceptance criteria forbid a zero-byte output. `inputDir` returns the directory relative paths resolve against: the file's own directory, or the **working directory** for stdin.
