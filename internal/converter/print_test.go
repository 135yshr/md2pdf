...
<body><h1>Heading</h1><p>Body text and 日本語.</p></body></html>`
	...
	if err := c.printPDF(t.Context(), htmlPath, pdfPath, nil); err != nil {
...
func TestPrintPDF_ReportsABadPageSizeBeforeLaunching(t *testing.T) {
	...
	err := c.printPDF(t.Context(), filepath.Join(t.TempDir(), "none.html"), filepath.Join(t.TempDir(), "out.pdf"), nil)
	if err == nil {
